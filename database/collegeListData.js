const collegeListData = [
  {
    collegeCode: "074",
    logoSrc: "./assets/images/College/dewan-thumb.jpeg",
    topRated: true,
    name: "Dewan VS Group of Institutions India",
    rating: 4.5,
    totalRatings: 245,
    star: 5,
    reviewCount: 243,
    location: "Bagpath Road, Meerut",
    contactNum: "849554111"
  },
  {
    collegeCode: "075",
    logoSrc: "./assets/images/College/jp-logo.jpg",
    topRated: true,
    name: "JP Institute of Engineering and Technology",
    rating: 3.1,
    totalRatings: 42,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "Mawana Road P.O Rajpura Meerut Uttar Pradesh Meerut ( Uttar Pradesh)",
    contactNum: ""  // Assuming not provided in the original data
  },
  {
    collegeCode: "",  // Missing in the original data
    logoSrc: "./assets/images/College/iimt-logo.jpg",
    topRated: true,
    name: "IIMT Group of Colleges, Meerut",
    rating: 4.5,
    totalRatings: 15,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "IIMT Nagar, ‘O’ Pocket, Ganga Nagar Colony, Mawana Road Meerut ( Uttar Pradesh)",
    contactNum: ""  // Assuming not provided in the original data
  },
  {
    collegeCode: "",  // Missing in the original data
    logoSrc: "./assets/images/College/sanskriti-logo.jpeg",
    topRated: true,
    name: "Sanskriti University",
    rating: 4.3,
    totalRatings: 191,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "28 K. M. Stone, Mathura - Delhi Highway, Chhata Mathura ( Uttar Pradesh)",
    contactNum: ""  // Assuming not provided in the original data
  },
  {
    collegeCode: "",  // Missing in the original data
    logoSrc: "./assets/images/College/asia-logo.jpeg",
    topRated: true,
    name: "Asia-Pacific Institute of Management, New Delhi",
    rating: 4.1,
    totalRatings: 85,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "3&4, GD Birla Marg, Institutional Area,Delhi",
    contactNum: ""  // Assuming not provided in the original data
  },
  {
    collegeCode: "",  // Missing in the original data
    logoSrc: "./assets/images/College/maharishi-logo.jpeg",
    topRated: true,
    name: "Maharishi University of Information Technology - Noida Campus",
    rating: 3.8,
    totalRatings: 17,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "Sector 110, PO- Maharishi Nagar, Distt. Gautam Budh Nagar Noida ( Uttar Pradesh)",
    contactNum: ""  // Assuming not provided in the original data
  },
  {
    collegeCode: "",  // Missing in the original data
    logoSrc: "./assets/images/College/iamr-logo.jpeg",
    topRated: true,
    name: "IAMR Group of Institutions",
    rating: 4.5,
    totalRatings: 5,
    star: 0,  // Assuming not provided in the original data
    reviewCount: 0,  // Assuming not provided in the original data
    location: "9th Km Stone, NH-58, Delhi-Meerut Road Ghaziabad ( Uttar Pradesh)",
    contactNum: ""  // Assuming not provided in the original data
  },
];

module.exports = collegeListData